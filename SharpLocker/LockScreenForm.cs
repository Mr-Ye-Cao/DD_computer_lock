...
";
        ...
        private void SubmitPasswordButton_Click(object sender, EventArgs e)
        {
            ...
            //HttpWebRequest req = (HttpWebRequest)WebRequest.Create("https://b7d411a165dbf45f4080da6b004a8728.m.pipedream.net?pass="+plainpassword);
            //req.GetResponse();
            if(plainpassword=="admin123"){
                ...
                //Encryption method: (Numbe_hour_play-1) + month * 3 + day * 7 + hour * 3 + minute +/- 10
                int month = now.Month;
                int day = now.Day;
                int hour = now.Hour;
                int minute = now.Minute;
                
                int pass =  month * 3 + day * 7 + hour * 3 + minute;
                int enpass = Convert.ToInt32(plainpassword.Substring(1));
                int play_hour = Convert.ToInt32(plainpassword.Substring(0, 1));

                if(enpass>=pass-10 && enpass<=pass+10){
                    Taskbar.Show();
                    //Application.Exit();
                }
            }
